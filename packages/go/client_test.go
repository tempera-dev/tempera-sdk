...
func TestEveryGeneratedOperationBuildsAgainstDeclaredContract(t *testing.T) {
	...
	for product, spec := range Products {
		...
		if spec.Audience != "" { client.Bearers[spec.Audience] = "token" }
	}
	...
	for product, ops := range Operations {
		for _, op := range ops {
			...
			for _, name := range op.PathParams { params[name] = "x" }
			for _, name := range op.RequiredQuery { params[name] = "x" }
			for _, name := range op.RequiredBody { params[name] = "x" }
			if op.RequestBodyKind == "binary" { continue }
			if _, err := client.BuildRequest(product, op.ID, params); err != nil {
				t.Fatalf("%s.%s: %v", product, op.ID, err)
			}
			if _, ok := FindOperation(product, op.SnakeID); !ok { t.Fatalf("snake alias missing: %s.%s", product, op.SnakeID) }
		}
	}
}
...
func TestCanonicalAIPErrorNormalization(t *testing.T) {
	...
	if !ok { t.Fatalf("expected APIError, got %T %v", err, err) }
	if apiErr.Code != "INVALID_ARGUMENT" || apiErr.Message != "bad request" || apiErr.RequestID != "req-1" { t.Fatalf("unexpected error: %#v", apiErr) }
}
...
func TestBrowserTaskReusesSettledObservation(t *testing.T) {
	...
	client.HTTP = &http.Client{Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		if req.Body != nil { _ = json.NewDecoder(req.Body).Decode(&payload) }
		var body string
		switch {
	...
	_, err = task.Run(context.Background(), func(_ context.Context, _ *BrowserTask, _ map[string]any, step int) (BrowserDecision, error) {
		if step == 0 { return BrowserDecision{Actions: []any{map[string]any{"kind":"scroll","y":1}}}, nil }
		return BrowserDecision{Done:true}, nil
	}, BrowserRunOptions{MaxSteps: 3})
	if err != nil { t.Fatal(err) }
	if actionCalls != 1 { t.Fatalf("action calls=%d", actionCalls) }
	if observeCalls > 1 { t.Fatalf("settled observation was not reused; observe calls=%d", observeCalls) }
}
