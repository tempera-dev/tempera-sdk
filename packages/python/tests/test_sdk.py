...
class TemperaSdkTest(unittest.TestCase):
    ...
    def test_scopes_match_the_control_plane_scope_registry(self):
        self.assertEqual(
            list(SCOPES),
            ["mcp:invoke", "memory:read", "memory:write", "memory:manage", "trace:read", "trace:write", "dataset:read", "dataset:write", "eval:run", "review:resolve", "workflow:read", "workflow:write", "workflow:run", "model:read", "model:invoke", "pii:unmask", "admin"],
        )
