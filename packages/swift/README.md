...
`OrdersCommerceClient` provides `offers(pageToken:pageSize:)`, `offer(id:)`, `saleOrders(pageToken:pageSize:)`, and `saleOrder(id:)`. Configure it with the provisioned Orders HTTPS origin, exact four-part `OrdersWorkspaceScope`, and the merchant ID returned by authenticated Payments workspace recovery. Its token closure must return an OAuth token with `orders:read` for the `tempera-dropshipping` audience. A Payments token cannot be reused for Orders. Native grant eligibility and hosted access require separate admission.
...
let page = try await orders.offers(pageSize: 50)
```
