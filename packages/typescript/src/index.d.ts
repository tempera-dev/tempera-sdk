...
export type TemperaAuthorizeUrlOptions = { issuerUrl: string; clientId: string; redirectUri: string; codeChallenge: string; audience?: TemperaAudience | (string & {}); scope?: TemperaScope | (string & {}) | readonly (TemperaScope | (string & {}))[]; state?: string };
export declare function buildAuthorizeUrl(options: TemperaAuthorizeUrlOptions): string;
export type TemperaTokenSet = { accessToken: string; refreshToken?: string; expiresIn?: number; scope?: string };
export type TemperaAuthOptions = { issuerUrl: string; clientId?: string; apiKey?: string; tokens?: Record<string, TemperaTokenSet>; fetch?: typeof fetch };
export declare class TemperaAuth {
  ...
  exchangeCode(options: { code: string; codeVerifier: string; redirectUri: string; audience?: TemperaAudience | (string & {}) }): Promise<TemperaTokenSet>;
  refresh(audience?: TemperaAudience | (string & {})): Promise<TemperaTokenSet>;
  revoke(audience?: TemperaAudience | (string & {}), options?: { tokenTypeHint?: "refresh_token" | "access_token" }): Promise<void>;
}
export declare const TEMPERA_PRODUCT_AUDIENCES: Readonly<Partial<Record<TemperaProductKey, { audience: TemperaAudience; env: string }>>>;

export type TemperaClientOptions = { auth?: TemperaAuth; accountToken?: string; introspectionSecret?: string; baseUrls?: Partial<Record<TemperaProductKey, string>>; environment?: TemperaEnvironment; fetch?: typeof fetch };
...
export type TemperaMcpClientOptions = { url?: string; auth?: TemperaAuth; bearer?: string; fetch?: typeof fetch };
