...
test("the product registry covers every Tempera product with palette included", () => {
  ...
  assert.equal(TEMPERA_PRODUCTS.temperaPayments.repository, "https://github.com/tempera-dev/tempera-payments");
...
test("audience-bearing products map to registered audiences", () => {
  ...
  assert.ok(TEMPERA_AUDIENCES.includes("tempera-payments"));
});
...
test("scopes match the control-plane scope registry", () => {
  assert.deepEqual(
    ...
    [
      ...
      "model:read", "model:invoke", "usage:reserve", "document:read", "document:write", "pii:unmask",
