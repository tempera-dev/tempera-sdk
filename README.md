...
- `contracts/palette-eval-openapi-operations.json` binds the three SDK methods
  ...
  responses, revision, and artifact digest. Refresh it only from the pinned
  generated spec with `python3 scripts/sync-palette-eval-openapi.py`; supplying
  `--source-checkout` additionally proves the clean checkout origin and `HEAD`.
