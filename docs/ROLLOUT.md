...
| tempo | `contracts/tempod.openapi.json` (generated from the runtime contract) | `scripts/export-tempod-openapi.sh --check` plus runtime-equality and route/spec tests in `tempo-headless` |
...
Tempo is pinned verbatim in `specs/tempo.openapi.json` with an adjacent exact
source lock. The normal sync path requires the producer commit to be reachable
from the named `origin/*` branch. `--allow-unpublished-local` exists only to
prepare a clean, exact, dependency-ordered review branch before the producer is
published; its lock is visibly marked `unpublished_local_review` and must be
regenerated without that flag before release.

```sh
python3 scripts/sync-tempo-openapi.py --check \
  --source-repo-dir ../tempo \
  --source-branch <reviewed-producer-branch> \
  --source-commit <40-character-producer-commit>
```
