...
```
product handler change                     (palette, tempo, cradle, remi, auth-hub)
...
| palette | `sdks/openapi/beater-api.json` (generated from `#[utoipa::path]`) | `scripts/check-contract-sync.sh` (CI: sdk-contract.yml) |
...
| remi | axum router in `crates/beater-memory/src/server.rs` (no spec) | `cargo test -p beater-memory` |
...
3. **Regenerate**: `python3 scripts/gen-sdk-surface.py` and commit the four
   generated files together with `surface.json`.
...
5. **Run the full gate**: `npm test` at the repo root. That runs, in order:
   - `scripts/check-sdk-surface.py`: surface.json invariants, regenerate-and-
     diff drift check, version-uniformity check, uniform-primitive markers.
...
The five products speak four different wire error shapes (see
